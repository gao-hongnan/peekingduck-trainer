"""IN CASE need to refer back to old working trainer."""
import os
import shutil
import time
...
training_logger = init_logger(
    log_file=Path.joinpath(logs_dir, "training.log"),
...
class Trainer:
    ...
    def __init__(
        ...
        wandb_run=None,
        early_stopping: callback.EarlyStopping = None,
    ):
        # Set params
    ...
    @staticmethod
    def valid_criterion(y_true: torch.Tensor, y_logits: torch.Tensor, criterion_params):
    ...
    def fit(
        ...
        for _epoch in range(1, self.params.epochs + 1):
            ...
            (valid_loss, valid_trues, valid_logits, valid_preds, valid_probs,) = (
            ...
            self.monitored_metric["metric_score"] = torch.clone(valid_macro_auroc)
