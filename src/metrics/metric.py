    ...
    if c_precision > 0 and c_recall > 0:
        result = (
            ...
            * (c_precision * c_recall)
            / (beta_squared * c_precision + c_recall + 1e-4)
        )
        return result
    else:
        return 0.0
...
def multiclass_roc_auc_score_torch(
    ...
    assert torch.isclose(
        macro_auroc,
        torchmetrics.AUROC(
            num_classes=pipeline_config.global_train_params.num_classes, average="macro"
        )(y_prob, y_true),
        rtol=1e-05,
        atol=1e-08,
        equal_nan=False,
...
def tp_fp_tn_fn_binary(
    ...
    if y_pred is not None and y_pred.dtype not in [torch.int64, torch.int32]:
        # raise dtype error
        raise TypeError(f"{y_pred.dtype} should be one of torch.int64 or torch.int32.")
    ...
    if y_pred is not None:
        ...
        macro_cm = torchmetrics.StatScores(reduce=reduce, num_classes=2)(y_pred, y_true)
    else:
        ...
        macro_cm_dict = {}
        y_true_binarized = multiclass_label_binarize_torch(y_true, class_labels, 1, 0)
        for label in class_labels:
